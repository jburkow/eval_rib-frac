...
Description: Takes a provided model predictions CSV file and applies an
    avalanche decision scheme to it. A new CSV file is created with the
    predictions that are kept after the scheme is applied.
'''
...
def apply_scheme(patient_df, thresholds):
...
def main(pargs):
    ...
    if pargs.method == "standard":
        test_thresholds = None
    else:
        test_thresholds = [pargs.base_val * val for val in [np.prod(np.array(avalanche_percentages[:k])) for k in range(1, len(avalanche_percentages)+1)]]
    ...
    for group_name, df_group in grouped_df:
        if pargs.method == "standard":
            scheme_df = df_group[df_group['Prob'] >= pargs.base_val]
        else:
            scheme_df = apply_scheme(df_group, test_thresholds)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Takes in a CSV file with model predictions and \
                outputs a new CSV with predictions that agree with the chosen avalanche scheme.')
