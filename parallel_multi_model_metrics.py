...
Description: Combines multiple model predictions and automatically calculates the avg +/- std.dev
    of precision, recall, and F2 scores from them all. Uses multiprocessing to speed up calculations.
'''
...
def parse_cmd_args():
    ...
    parser.add_argument('--max_combos', type=int, help='Maximum number of combinations to calculate.')
...
def calc_metrics(
    ...
    for _, patient in tqdm(enumerate(match_annos), desc='Calculating Metrics', total=len(match_annos), disable=True):
        ...
        new_df_row = pd.DataFrame({'Patient': [patient],
                                   ...
                                   'True Negatives': [true_neg]}, index=[0])
